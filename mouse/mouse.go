package mouse // import "engo.io/engo/mouse"
...
func (m *MouseSystem) Add(basic *ecs.BasicEntity, mouse *MouseComponent, space *space.SpaceComponent, render *render.RenderComponent) {
...
func (m *MouseSystem) Update(dt float32) {
	...
	m.mouseX = internalinput.Mouse.X*internalcamera.Cam.Z()*(window.Width()/window.WindowWidth()) + internalcamera.Cam.X() - (window.Width()/2)*internalcamera.Cam.Z()
	m.mouseY = internalinput.Mouse.Y*internalcamera.Cam.Z()*(window.Height()/window.WindowHeight()) + internalcamera.Cam.Y() - (window.Height()/2)*internalcamera.Cam.Z()
	...
	for _, e := range m.entities {
		...
		*e.MouseComponent = MouseComponent{
		...
		if e.RenderComponent != nil {
			...
			if e.RenderComponent.Shader == shaders.HUDShader {
				mx = internalinput.Mouse.X
				my = internalinput.Mouse.Y
			}
		}
		...
		if e.MouseComponent.Track ||
			...
			switch internalinput.Mouse.Action {
			case input.PRESS:
				switch internalinput.Mouse.Button {
		...
		e.MouseComponent.Modifier = internalinput.Mouse.Modifer
	}

	// reset mouse.Action value to something meaningless to avoid
	// catching the same "signal" twice
	internalinput.Mouse.Action = input.NEUTRAL
}
