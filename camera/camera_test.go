...
func initialize() {
	...
	internalwindow.WorldBounds = AABB{Point{0, 0}, Point{300, 300}}
	internalengo.CurrentWorld = &ecs.World{}

	cam = &CameraSystem{}
	cam.New(internalengo.CurrentWorld)
}
...
func TestCameraMoveX(t *testing.T) {
	...
	cam.moveX(305)
	assert.Equal(t, cam.X(), internalwindow.WorldBounds.Max.X, "Moving too many unit, should have moved the camera to the maximum")

	cam.moveX(-305)
	assert.Equal(t, cam.X(), internalwindow.WorldBounds.Min.X, "Moving too many units back, should have moved the camera to the minimum")
}
...
func TestCameraMoveY(t *testing.T) {
	...
	cam.moveY(305)
	assert.Equal(t, cam.Y(), internalwindow.WorldBounds.Max.Y, "Moving too many unit, should have moved the camera to the maximum")

	cam.moveY(-305)
	assert.Equal(t, cam.Y(), internalwindow.WorldBounds.Min.Y, "Moving too many units back, should have moved the camera to the minimum")
}
...
func TestCameraMoveToX(t *testing.T) {
	...
	cam.moveToX(600)
	assert.Equal(t, cam.X(), internalwindow.WorldBounds.Max.X, "Moving to a location out of bounds, should get us to the maximum")

	cam.moveToX(-10)
	assert.Equal(t, cam.X(), internalwindow.WorldBounds.Min.X, "Moving to a location out of bounds, should get us to the minimum")
}
...
func TestCameraMoveToY(t *testing.T) {
	...
	cam.moveToY(600)
	assert.Equal(t, cam.Y(), internalwindow.WorldBounds.Max.Y, "Moving to a location out of bounds, should get us to the maximum")

	cam.moveToY(-10)
	assert.Equal(t, cam.Y(), internalwindow.WorldBounds.Min.Y, "Moving to a location out of bounds, should get us to the minimum")
}
