package font // import "engo.io/engo/font"
...
// TODO(u): Figure out how to implement CreatePreloaded without cycles.
...
func (f *Font) CreatePreloaded() error {
	var ok bool
	f.TTF, ok = assets.Files.fonts[f.URL]
	if !ok {
...
func (f *Font) Render(text string) *texture.Texture {
	nrgba := f.RenderNRGBA(text)

	// Create texture
	imObj := engoimage.NewImageObject(nrgba)
	return texture.NewTexture(imObj)
